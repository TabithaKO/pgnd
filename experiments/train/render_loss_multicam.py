"""
render_loss_ablation2.py — Neural mesh renderer render loss (MULTI-CAMERA)
==========================================================================

Multi-camera extension: renders from ALL specified cameras each step.
The same mesh is built once, then rendered from each viewpoint independently.
The MLP's view-dependent features (N·V, view direction) naturally produce
different outputs per camera — this is how it learns view-dependent rendering.

Key change from single-camera version:
    - __init__() accepts camera_ids=[0, 1] for multi-camera
    - setup_episode() loads cam_settings + gt_loader for EACH camera
    - compute_loss() builds mesh once, renders from all cameras, averages losses
    - Backward compatible: single camera_id still works

No changes needed to neural_mesh_renderer.py — it already accepts cam_settings
per forward() call and computes view-dependent features.
"""
...
try:
    ...
    print("[render_loss_ablation2] Warning: Could not import neural_mesh_renderer")
...
# =============================================================================
# Multi-Camera Render Loss Module
# =============================================================================

class RenderLossModuleAblation2:
    """Render loss using a neural mesh renderer with multi-camera support.

    Renders from ALL specified cameras each step. The same mesh is built once,
    then the MLP produces view-dependent Gaussian params per camera via its
    view_dir and N·V features.

    Manages:
        - NeuralMeshRenderer (persistent, learned MLP)
        - Coordinate transforms (per episode, shared across cameras)
        - GT image loading (per episode, per camera)
        - Mesh building (once per compute_loss call, shared across cameras)
        - Loss computation (averaged across cameras)
    """
    ...
    def __init__(
        ...
        # Backward compat: single camera_id attribute
        self.camera_id = self.camera_ids[0]

        print(f'[RenderLossAblation2] Cameras: {self.camera_ids}')
        ...
        # Multi-camera: list of per-camera dicts
        self.cameras = []  # [{'camera_id': int, 'cam_settings': dict, 'gt_loader': GTImageLoader}, ...]
        # Backward compat: single-camera attributes (point to first camera)
    ...
    def setup_episode(
        ...
        Loads per-episode coordinate transform (shared) and per-camera
        settings + GT image loaders for ALL cameras in self.camera_ids.
        ...
        Returns:
            True if render loss is active for this episode (≥1 camera loaded)
        """
        cfg = self.cfg
        self.active = False
        self.cameras = []
        ...
        # =====================================================================
        # Step 2: Setup coordinate transforms (per-episode, shared across cameras)
        # =====================================================================
        ...
        # =====================================================================
        # Step 3: Load camera parameters for ALL cameras
        # =====================================================================
        ...
        for cam_id in self.camera_ids:
            try:
                ...
                print(f"  Camera {cam_id} loaded ✓")

            except FileNotFoundError as e:
                print(f"  ⚠️  Camera {cam_id} skipped: {e}")
            except Exception as e:
                print(f"  ⚠️  Camera {cam_id} failed: {e}")

        if not self.cameras:
            print(f"  ⚠️  No cameras loaded")
            return False

        # Backward compatibility: single-camera attributes point to first camera
        self.cam_settings = self.cameras[0]['cam_settings']
        self.gt_loader = self.cameras[0]['gt_loader']

        self.active = True
        cam_ids_str = ','.join(str(c['camera_id']) for c in self.cameras)
        print(f"  ✅ Render loss active (neural mesh renderer, cameras=[{cam_ids_str}])")
        return True
    ...
    def compute_loss(
        ...
        """Compute render loss for a rollout step across ALL cameras.

        Pipeline:
        1. Build mesh ONCE from current particles
        2. For EACH camera:
           a. Project vertex colors from that camera's GT image
           b. Run neural renderer MLP forward (view-dependent output)
           c. Rasterize to image
           d. Compute image loss vs GT
        3. Average losses across cameras

        Gradients flow to:
        - Dynamics model: through particle positions → mesh → Gaussian positions (all cameras)
        - Renderer MLP: through predicted colors/scales/opacities (if not frozen)
        ...
        Returns:
            (loss_render, None): loss_render includes lambda_render scaling, averaged over cameras
        """
        ...
        # =====================================================================
        # Step 1: Build mesh ONCE (shared across all cameras)
        # =====================================================================
        ...
        # Use ORIGINAL particles (with grad) as vertices
        vertices = particles[:n_verts]  # (N_verts, 3) — WITH gradient

        # =====================================================================
        # Step 2: Render from EACH camera and compute per-camera loss
        # =====================================================================
        camera_losses = []
        first_rendered = None  # For debug viz
        first_gt = None
        first_mask = None

        for cam_info in self.cameras:
            cam_settings = cam_info['cam_settings']
            gt_loader = cam_info['gt_loader']
            cam_id = cam_info['camera_id']

            # Load GT image for this camera
            gt_image = gt_loader.load_frame(rollout_step)
            if gt_image is None:
                continue

            # Project vertex colors from THIS camera's GT image
            vertex_colors = project_vertex_colors(
                vertices_preproc=vertices.detach(),
                image=gt_image,
                cam_settings=cam_settings,
                coord_transform=self.coord_transform,
            )  # (N_verts, 3) — no grad (input features)

            # Neural renderer forward (view-dependent via cam_settings)
            ...
            # Load mask
            gt_mask = gt_loader.load_mask(rollout_step)
            ...
            camera_losses.append(loss_image)

            # Save first camera's render for debug
            if first_rendered is None:
                first_rendered = rendered_image
                first_gt = gt_image
                first_mask = gt_mask

        if not camera_losses:
            return None, None

        # =====================================================================
        # Step 3: Average across cameras
        # =====================================================================
        loss_avg = sum(camera_losses) / len(camera_losses)
        loss_render = loss_avg * self.lambda_render

        # =====================================================================
        # Debug visualization (first camera only)
        # =====================================================================
        if self._debug_save_counter % self._debug_save_interval == 0:
            if first_rendered is not None:
                self._save_debug_images(
                    first_rendered, first_gt, first_mask, rollout_step,
                    particles=particles, debug_info=None,
                    n_cameras=len(camera_losses),
                )
    ...
    def _save_debug_images(
        ...
        try:
            with torch.no_grad():
                ...
                cv2.putText(canvas_uint8, f'Rendered ({n_g}G/{n_f}F, {n_cameras}cam)', (10, 22), font, 0.5, (255,255,255), 1)
                    ...
                    if wandb.run is not None:
                        wandb.log({
                            'render_debug_ablation2/comparison': wandb.Image(
                                canvas_uint8,
                                caption=f'Neural Mesh | GT | Diff (step {rollout_step}, {n_cameras} cameras)'
                            ),
                        }, commit=False)
                except ImportError:
                    pass
...
def create_render_loss_module_ablation2(cfg, log_root, **kwargs) -> RenderLossModuleAblation2:
    ...
    Supports multi-camera via camera_ids=[0, 1] kwarg.
    Falls back to single camera_id for backward compatibility.
    """
