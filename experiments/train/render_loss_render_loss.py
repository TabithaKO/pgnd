...
def masked_image_loss(
    ...
    """Combined L1 + SSIM image loss, properly normalized for masked regions.
    ...
    if mask is not None:
        ...
        l1 = (pred_masked - gt_masked).abs().sum() / num_mask_pixels
        
        # SSIM: computed on masked images (still full-resolution for windowing)
        ssim = ssim_loss(pred_masked, gt_masked)
    else:
...
class RenderLossModule:
    ...
    def __init__(
        ...
        self._debug_save_interval = 1  # save every render for debugging
    ...
    def compute_loss(
        ...
        if self._debug_save_counter == 0:
            print(f'[render_loss] DEBUG coord check:')
            print(f'  gs_xyz_new (preproc): min={gs_xyz_new.detach().min(0).values.cpu().numpy()}, max={gs_xyz_new.detach().max(0).values.cpu().numpy()}')
            print(f'  gs_xyz_render (orig): min={gs_xyz_render.detach().min(0).values.cpu().numpy()}, max={gs_xyz_render.detach().max(0).values.cpu().numpy()}')
            # Expected original: min~[0.15, 0.30, -0.31], max~[0.57, 0.80, 0.04]
    ...
    def _save_debug_images(
        ...
        """Save side-by-side debug images: rendered | GT | masked GT | diff."""
        try:
            with torch.no_grad():
                ...
                if mask is not None:
                    ...
                    masked_rendered_np = rendered_np * mask_np
                else:
                    mask_np = np.ones_like(gt_np[:, :, :1])
                    masked_gt_np = gt_np
                    masked_rendered_np = rendered_np
                
                # Difference map (amplified for visibility)
                diff = np.abs(masked_rendered_np - masked_gt_np)
                diff_amplified = np.clip(diff * 5.0, 0, 1)
                
                # Side-by-side: rendered | masked GT | masked rendered | diff
                h, w = rendered_np.shape[:2]
                gap = 2
                canvas = np.ones((h + 30, w * 4 + gap * 3, 3), dtype=np.float32)  # white bg
                
                # Row of images
                y0 = 30  # space for labels
                canvas[y0:y0+h, 0:w] = rendered_np
                canvas[y0:y0+h, w+gap:2*w+gap] = masked_gt_np
                canvas[y0:y0+h, 2*w+2*gap:3*w+2*gap] = masked_rendered_np
                canvas[y0:y0+h, 3*w+3*gap:4*w+3*gap] = diff_amplified
                
                canvas_uint8 = (canvas * 255).astype(np.uint8)
                
                # Add labels with cv2
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(canvas_uint8, 'GS Rendered', (10, 22), font, 0.6, (255,255,255), 1)
                cv2.putText(canvas_uint8, 'GT Masked', (w+gap+10, 22), font, 0.6, (255,255,255), 1)
                cv2.putText(canvas_uint8, 'Render Masked', (2*w+2*gap+10, 22), font, 0.6, (255,255,255), 1)
                cv2.putText(canvas_uint8, 'Diff (5x)', (3*w+3*gap+10, 22), font, 0.6, (255,255,255), 1)
