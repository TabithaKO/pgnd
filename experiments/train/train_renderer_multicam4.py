...
Usage:
    ...
    python train_renderer_multicam.py \
        --baseline_ckpt cloth/train/ckpt/100000.pt \
        --episodes 162-241 \
        --num_iterations 500000 \
        --output_name cloth/renderer_v4d_multicam_4cam \
        --camera_ids 0,1,2,3 \
        --patience 30000 \
...
# Global camera IDs — set from args in main()
CAMERA_IDS = [0, 1, 2, 3]
...
from train.render_loss_multicam import PGNDCoordinateTransform
...
def train_renderer(
    ...
    if resume_ckpt is not None:
        ...
        if ckpt_path.exists():
            ...
            wandb_resume_id = ckpt.get('wandb_run_id', None)
            ...
            if wandb_resume_id:
                print(f'[resume] Will resume wandb run: {wandb_resume_id}')
        else:
    ...
    def get_episode_setup(episode: int):
        ...
        try:
            ...
            # Multi-camera: load ALL cameras for view-dependent training
            camera_ids = CAMERA_IDS
            cameras = []
            for cid in camera_ids:
                try:
                    R_c = cv2.Rodrigues(rvec[cid])[0]
                    t_c = tvec[cid, :, 0]
                    ...
                    print(f'  [setup] Camera {cid} failed for episode {episode}: {cam_e}')
            ...
            setup = {
                ...
                'cameras': cameras,
                # Backward compat
                'cam_settings': cameras[0]['cam_settings'],
                'gt_loader': cameras[0]['gt_loader'],
            }
    ...
    eval_window = 5000  # evaluate early stopping every this many iters
    eval_losses = []  # collect losses for windowed average
    print(f'[early stopping] patience={patience}, min_iterations={min_iterations}, eval_window={eval_window}')
    ...
    for iteration in range(start_iteration, num_iterations + 1):
        ...
        # Multi-camera: randomly pick a camera for this iteration
        cameras = setup.get('cameras', None)
        if cameras and len(cameras) > 1:
            cam_info = cameras[np.random.randint(len(cameras))]
        ...
        if iteration > 0 and iteration % eval_window == 0 and len(eval_losses) > 0:
            window_avg = np.mean(eval_losses)
            eval_losses = []
            if window_avg < best_avg_loss:
                best_avg_loss = window_avg
                best_iteration = iteration
                best_state_dict = {k: v.clone() for k, v in renderer.state_dict().items()}
                print(f'  [early stopping] New best: loss={best_avg_loss:.6f} at iter {iteration}')
            elif iteration >= min_iterations and (iteration - best_iteration) >= patience:
                print(f'\n{"="*60}')
                print(f'EARLY STOPPING at iteration {iteration}')
                print(f'  Best loss: {best_avg_loss:.6f} at iteration {best_iteration}')
                print(f'  No improvement for {iteration - best_iteration} iterations (patience={patience})')
                print(f'{"="*60}')
                # Save best checkpoint
                best_ckpt_path = ckpt_dir / f'{best_iteration:06d}_best.pt'
                torch.save({
                    'renderer': best_state_dict,
                    'iteration': best_iteration,
                    'num_states': len(filtered_states),
                    'best_loss': best_avg_loss,
                }, str(best_ckpt_path))
                print(f'  Saved best checkpoint: {best_ckpt_path}')
                # Also save as the standard final checkpoint
                ...
                if use_wandb:
                    wandb.log({'early_stopped': True, 'best_iteration': best_iteration}, step=iteration)
                    wandb.finish()
                return
...
def main():
    ...
    parser.add_argument('--camera_ids', type=str, default='0,1,2,3',
                        help='Comma-separated camera IDs to train on (default: 0,1,2,3)')
    parser.add_argument('--patience', type=int, default=30000,
                        help='Early stopping patience in iterations (stop if no improvement for this many iters)')
    parser.add_argument('--min_iterations', type=int, default=50000,
                        help='Minimum iterations before early stopping can trigger')

    global args
    args = parser.parse_args()

    # Set global camera IDs
    global CAMERA_IDS
